...
export default class Serialize extends Client {
  ...
  async serial(client, msg) {
    ...
    m.body =
      m.type === "conversation"
        ...
        ? msg.message?.listResponseMessage?.singleSelectReply?.selectedRowId ||
          ...
          msg.text
        : "";
    m.mentions = msg.message[m.type]?.contextInfo
      ? msg.message[m.type]?.contextInfo.mentionedJid
      : [];
    if (msg.message[m.type]?.contextInfo?.quotedMessage) {
      ...
      m.quoted.download = (path = null) =>
        this.downloadMedia(m.quoted.message, path);
    } else {
      m.quoted = null;
    }
    if (m.isGroupMsg) {
      ...
      m.group.admins = m.group.groupMetadata.participants
    ...
    m.command = m.isCmd
      ? m.body.slice(1).trim().split(/ +/).shift().toLowerCase()
      : null;
    m.msg = msg;
